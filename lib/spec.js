...
(function() {
  ...
  function eq(left, right, stack) {
    var className, property, size, sizeRight, result;
  ...
  Test.prototype.ok = function(expression, data) {
    var isData = typeof data == 'object' && data, event = {
      // Convert a data object into an event object.
      'actual': (isData && 'actual' in data) ? data.actual : expression,
      'expected': (isData && 'expected' in data) ? data.expected : true,
      'message': (isData ? data.message : data)
    };
    if (typeof event.message != 'string' || !event.message) event.message = 'ok';
  ...
  Test.prototype.deepEqual = function(actual, expected, message) {
    return this.ok(eq(actual, expected, []), {
      'actual': actual,
      'expected': expected,
  ...
  Test.prototype.raises = function(block, expected, message) {
    ...
    return this.ok(ok, {
      'actual': block,
      'expected': expected,
      'message': typeof message == 'string' && message || 'raises'
    });
  };
  ...
  Spec.prototype.bind = Test.prototype.bind = function(type, listener) {
  ...
  Spec.prototype.unbind = Test.prototype.unbind = function(type, listener) {
