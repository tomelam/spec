...
;(function (root, Spec) {
  ...
  all = exports.all = (function () {
    ...
    if (!size) {
      ...
      all = function all(object, callback, context) {
        var property, length;
        if (object !== Object(object)) {
          throw TypeError("Invalid argument.");
        }
        for (property in object) {
          if (hasKey(object, property) && !callback.call(context, property, object[property], object)) {
            return false;
          }
        }
      ...
      all = function all(object, callback, context) {
        ...
        for (property in object) {
          ...
          if (!(isFunction && property === "prototype") && !hasKey(memo, property) && (memo[property] = 1) && hasKey(object, property) && !callback.call(context, property, object[property], object)) {
            return false;
          }
        }
        return true;
      ...
      all = function all(object, callback, context) {
        ...
        isFunction = getClass.call(object) == "[object Function]";
        for (property in object) {
          if (!(isFunction && property === "prototype") && hasKey(object, property) && !(isConstructor = property === "constructor") && !callback.call(context, property, object[property], object)) {
            return false;
          }
        }
        // Manually invoke the callback for the `constructor` property due to
        // cross-environment inconsistencies.
        return (isConstructor || hasKey(object, "constructor")) ? !!callback.call(context, "constructor", object.constructor, object) : true;
      };
    }
    return all;
  })(),
    ...
    function eq(left, right, stack) {
      ...
      if (className == "[object Array]") {
        ...
        result = all(left, function (key, value) {
          ...
          return hasKey(right, key) && eq(value, right[key], stack);
        });
  ...
  forEach = Spec.forEach = function forEach(list, callback, context) {
    var index, length;
    if (Object(list) !== list) {
      throw TypeError("Invalid argument.");
    }
    for (index = 0, length = list.length >>> 0; index < length; index += 1) {
      if (callback.call(context, list[index], index, list) === false) {
        break;
      }
    }
    return list;
  ...
  function addListener(event, callback, context) {
    if (event && callback) {
      forEach(event.split(" "), function (event) {
        var callbacks = this.events[event], target = callbacks ? callbacks.previous : {}, previous;
  ...
  function removeListener(event, callback, context) {
    if (event && callback) {
      forEach(event.split(" "), function (event) {
        var target = this.events[event], previous;
        if (target) {
          // Create a new registry without the given event handler.
  ...
  // or all handlers for all events if the `event` is omitted.
  Events.prototype.removeAllListeners = removeAllListeners;
  ...
  function runSuite() {
    ...
    function onEvent(event) {
      ...
      switch (event.type) {
        ...
        case "teardown":
          ...
          if ((position = suite.index(position += 1)) != null) {
    ...
    if ((position = suite.index(position)) != null) {
      target = suite[position];
      defer(target.on("all", onEvent).run, target);
    } else {
